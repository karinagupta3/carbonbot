...
class Miriel:
    def __init__(self, api_key, base_url="https://api.prod.miriel.ai", verify=True):
    ...
    def learn(self, input, user_id=None, metadata=None, discoverable=True, grant_ids=["*"], domain_restrictions=None, recursion_depth=0, priority=100, project=None, wait_for_complete=False, chunk_size=None):
        """Add a string, url, or file to the Miriel AI system for learning"""
        is_file = isinstance(input, str) and os.path.exists(input)
        ...
        if is_file:
            endpoint = f"{self.base_url}/api/v2/upload_files"
            files_list = []
